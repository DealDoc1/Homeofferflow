...
class AgentLandingFunnelTests(unittest.TestCase):
    ...
    def test_personal_agent_workspace_keeps_responsibilities_clear_without_a_brokerage_gate(self):
        ...
        self.assertIn('placeholder="agent@example.com"', INDEX)
        self.assertNotIn('placeholder="agent@brokerage.com"', INDEX)
    ...
    def test_purchase_interview_keeps_common_addenda_in_the_guided_handoff(self):
        ...
        interview = INDEX[start:end]
        self.assertIn("Prepare another purchase document", interview)
        self.assertIn("Which purchase document does this transaction need?", interview)
        for label, opener in (
    ...
    def test_public_agent_copy_matches_the_transaction_first_activation_path(self):
        ...
        self.assertIn('Paid plan terms appear before recurring checkout.', AGENTS)
        focus = (ROOT / "assets" / "agent-landing-focus.js").read_text(encoding="utf-8")
        self.assertIn('No brokerage seat or payment required to start.', focus)
        self.assertIn('Paid plan terms appear before recurring checkout.', focus)
    ...
    def test_agent_faq_explains_the_transaction_specific_next_step(self):
        ...
        self.assertIn('<strong>Tenant representation</strong> goes directly to choosing the representation agreement that fits the tenant.', AGENTS)
    ...
    def test_agent_landing_cards_and_structured_data_match_the_question_two_interview(self):
        ...
        self.assertIn('Choose the representation agreement that fits this tenant.', AGENTS)
