...
def valid_residential_lease_payload():
    return {
        "formCode": "TXR-1953",
        "formSourceId": "00000000-0000-0000-0000-000000000001",
        "propertyAddress": "1438 Whitaker Road, Van Alstyne, TX",
        "buyerNames": ["Test Buyer"],
        "sellerNames": ["Test Seller"],
        "leaseStatus": "assignment",
        "deliveryChoice": "not_received",
        "deliveryDays": "3",
        "explanation": "No tenant disputes reported.",
        "residentialLeaseReviewAcknowledgment": True,
    }
...
class StandaloneAgreementFoundationTests(unittest.TestCase):
    ...
    def test_residential_lease_draft_requires_conditional_delivery_choices(self):
        draft = MODULE._parse_txr_1953_draft(valid_residential_lease_payload())
        self.assertEqual(draft["agreement_data"]["delivery_days"], "3")
        payload = valid_residential_lease_payload()
        payload["deliveryDays"] = ""
        with self.assertRaisesRegex(ValueError, "delivery time"):
            MODULE._parse_txr_1953_draft(payload)
        payload = valid_residential_lease_payload()
        payload["leaseStatus"] = "termination"
    ...
    def test_agent_ui_requires_an_approved_source_and_prepares_reviewable_documents(self):
        ...
        self.assertIn("background form-library failure", HTML)
        ...
        self.assertIn("We couldn’t open that document right now", status_copy)
        ...
        self.assertIn("Choose whether you or the broker will sign this agreement with the client.", HTML)
        self.assertIn("Choose whether you or the broker will sign this notice with the consumer.", HTML)
        self.assertIn("Choose whether you or the broker will acknowledge this showing form.", HTML)
        self.assertNotIn("HomeOfferFlow will not guess", HTML)
        self.assertNotIn("HomeOfferFlow will not infer", HTML)
        ...
        self.assertIn('We couldn’t prepare this document. Check your entries and try again.', HTML)
        for form_id in ('txr1914AgreementForm', 'txr1917AgreementForm', 'txr1919AgreementForm'):
            self.assertIn(form_id, HTML)
        self.assertIn("button.textContent = hasSignatureQueue(form) ? 'Preparing document…' : 'Saving for review…';", HTML)
        self.assertIn("button.textContent = hasSignatureQueue(form) ? 'Ready to review' : 'Saved for review';", HTML)
        ...
        self.assertIn("submit.textContent = 'Saving your document…'", HTML)
        self.assertIn("button.textContent = hasSignatureQueue(form) ? 'Prepare document' : 'Save for review';", HTML)
        self.assertGreaterEqual(HTML.count("submit.setAttribute('aria-busy', 'true')"), 3)
        self.assertGreaterEqual(HTML.count("submit.textContent = 'Saving your document…'"), 1)
        self.assertIn("Could not prepare the showing document.", HTML)
    ...
    def test_agents_can_only_view_their_own_saved_draft_summaries(self):
        ...
        self.assertIn("Review your saved documents here before choosing the next step for each one.", HTML)
