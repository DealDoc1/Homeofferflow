...
class SignwellSendContractTests(unittest.TestCase):
    ...
    def exercise_send(self, kind, changed_field=False):
        created = {}
        calls = []
        client = AsyncMock()
        ...
        rows = [[agreement], [source]] if kind == 'agreement' else [[disclosure]]
        with patch.object(MODULE, 'TXR_SIGNING_ENABLED', True), \
             ...
             patch.object(MODULE, '_get', AsyncMock(side_effect=rows)), \
            ...
            if kind == 'agreement':
                operation = MODULE._send_txr_agreement_for_signature(USER, {
                    'agreementId': RECORD_ID, 'clientEmails': ['client@example.com'],
                    'confirmedRecipients': recipients,
                })
            else:
                operation = MODULE._send_seller_disclosure_for_signature(USER, {
                    'draftId': RECORD_ID, 'signerEmails': ['seller@example.com', 'buyer@example.com'],
                })
            if changed_field:
                with self.assertRaisesRegex(RuntimeError, 'Nothing was sent'):
                ...
                self.assertEqual(calls, ['create', 'inspect', 'send'])
                self.assertEqual(persist.call_args.args[2]['status'], 'sent')
                self.assertEqual(persist.call_args.args[2]['signwell_document_id'], 'provider-id')
