...
class SubscriptionCheckoutFunnelMetricTests(unittest.TestCase):
    ...
    def test_ondemand_checkout_uses_the_shared_privacy_safe_funnel_events(self):
        ...
        self.assertIn('metadata: { source: "ondemand", plan: "agent", billing: "monthly", channel, ...(campaign ? {utmCampaign: campaign} : {}) }', source)
    ...
    def test_ondemand_new_offer_returns_to_the_transaction_picker_before_a_draft(self):
        ...
        self.assertIn("if (action === 'transaction_start') {\n      if (role === 'investor') {", source)
        self.assertIn("recordOndemandTransactionPickerOpened();", source)
