...
class TxrSignerGeometryTests(unittest.TestCase):
    ...
    def test_corrected_signature_rows_clear_the_caption_baseline(self):
        ...
        cases = (
            ...
            (build_signwell_fields_txr1507, FORM_CASES[2][3], {
                "txr1507_associate_signature_p2": 533.95 / .75,
                "txr1507_associate_date_p2": 533.95 / .75,
                "txr1507_client1_signature_p2": 533.95 / .75,
                "txr1507_client1_date_p2": 533.95 / .75,
                "txr1507_client2_signature_p2": 616.78 / .75,
                "txr1507_client2_date_p2": 616.78 / .75,
            }),
        )
    ...
    def test_txr1501_second_client_and_txr1507_associate_use_their_actual_rules(self):
        ...
        for field_id, expected in {
            "txr1507_client1_signature_p2": (432, 684, 240, 708),
            "txr1507_client1_date_p2": (696, 692, 72, 710),
            "txr1507_client2_signature_p2": (432, 794, 240, 818),
            "txr1507_client2_date_p2": (696, 802, 72, 820),
        }.items():
        ...
        self.assertEqual(txr1507["txr1507_client1_initials_p1"]["x"], 543)
    ...
    def test_addendum_signatures_end_at_their_source_rules(self):
        ...
        Rechecked TXR-1905/1914 candidates finish within two PDF points above
        the precise rule. Other maps retain their existing rounded positions.
        """
        cases = (
                ...
                for party in ("buyer", "seller"):
                    ...
                    for field, bottom in ((first, first_bottom), (second, second_bottom)):
                        if prefix in ('txr1905', 'txr1914', 'txr1917'):
                            ...
                            self.assertEqual(field['y'] + field['height'], bottom)
