...
class SellerDisclosureFeedbackTests(unittest.TestCase):
    ...
    def test_preview_and_actions_use_accessible_status_feedback(self):
        ...
        self.assertIn("saveButton.textContent = 'Saving…'", script)
        self.assertIn("saveButton.disabled = false", script)
        self.assertIn("saveButton.textContent = 'Save and preview'", script)
        self.assertIn("async function previewSellerDisclosure", script)
        self.assertIn("await previewSellerDisclosure(card.dataset.sellerDraftId, saveButton)", script)
        ...
        self.assertIn("Sign in before saving this seller disclosure.", script)
        self.assertIn("Sign in before sending the seller review email.", script)
        self.assertNotIn("alert(error.message || 'Private seller preview is unavailable.')", script)
        self.assertNotIn("return alert('Sign in before saving this seller disclosure.')", script)
        self.assertNotIn("return alert('Sign in before sending the seller review email.')", script)
