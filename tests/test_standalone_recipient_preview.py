...
class StandaloneRecipientPreviewTests(unittest.TestCase):
    def test_preview_includes_the_actual_account_associate_and_scopes_the_draft(self):
        get = AsyncMock(side_effect=[[AGENT_PROFILE], [showing_draft()], [{"name": "Office"}]])
        ...
        self.assertIn(f"id=eq.{AGREEMENT_ID}", get.call_args_list[1].args[0])
        self.assertIn("agent_user_id=eq.owner-1", get.call_args_list[1].args[0])
        self.assertIn("status=eq.draft", get.call_args_list[1].args[0])
    ...
    def test_broker_plan_displays_the_broker_instead_of_the_requesting_agent(self):
        get = AsyncMock(side_effect=[[AGENT_PROFILE], [showing_draft("broker_and_clients")], [
    ...
    def test_purchase_addenda_do_not_add_an_agent_or_query_broker_contacts(self):
        ...
        get = AsyncMock(side_effect=[[AGENT_PROFILE], [draft]])
        profiles = AsyncMock()
        with patch.object(MODULE, "_get", get), patch.object(MODULE, "_get_optional", profiles):
            result = asyncio.run(MODULE._standalone_signing_recipient_preview(USER, AGREEMENT_ID))
    ...
    def test_unavailable_or_other_owner_draft_does_not_expose_signing_contacts(self):
        get = AsyncMock(side_effect=[[AGENT_PROFILE], []])
        profiles = AsyncMock()
        with patch.object(MODULE, "_get", get), patch.object(MODULE, "_get_optional", profiles):
    ...
    def test_consumer_role_cannot_load_signing_contacts(self):
        get = AsyncMock(return_value=[{"role": "homebuyer"}])
        with patch.object(MODULE, "_get", get), self.assertRaisesRegex(PermissionError, "Agent or broker access"):
            asyncio.run(MODULE._standalone_signing_recipient_preview(USER, AGREEMENT_ID))
