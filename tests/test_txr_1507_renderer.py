...
class Txr1507RendererTests(unittest.TestCase):
    ...
    def test_signer_map_is_separate_for_one_and_two_clients(self):
        ...
        self.assertEqual(next(field["y"] for field in two if field["api_id"] == "txr1507_associate_signature_p2"), 695)
        self.assertEqual(next(field["y"] for field in two if field["api_id"] == "txr1507_client2_signature_p2"), 804)
        self.assertEqual(next(field["x"] for field in two if field["api_id"] == "txr1507_client1_signature_p2"), 450)
