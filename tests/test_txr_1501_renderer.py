...
class Txr1501RendererTests(unittest.TestCase):
    ...
    def test_signer_map_requires_plan_and_supports_one_or_two_clients(self):
        ...
        self.assertEqual(next(field["y"] for field in two if field["api_id"] == "txr1501_client1_signature_p6"), 590)
        self.assertEqual(next(field["y"] for field in two if field["api_id"] == "txr1501_client2_signature_p6"), 700)
        self.assertEqual(next(field["y"] for field in two if field["api_id"] == "txr1501_associate_signature_p6"), 590)
