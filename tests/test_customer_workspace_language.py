...
class CustomerWorkspaceLanguageTests(unittest.TestCase):
    ...
    def test_agent_forms_explain_review_before_available_sending(self):
        ...
        self.assertIn("const canSend = signingEnabled && signingFormCodes.has(agreement.form_code) && !agreement.signwell_document_id && (agreement.status === 'draft' || canRetryUnsent);", HTML)
