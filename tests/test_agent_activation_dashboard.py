...
class AgentActivationDashboardTests(unittest.TestCase):
    ...
    def test_transaction_interview_recommends_a_package_before_opening_a_workspace(self):
        ...
        self.assertIn("Prepare a purchase offer", HTML)
        self.assertIn("We’ll prepare the supported documents your answers require.", HTML)
        self.assertIn("Set up buyer representation", HTML)
        self.assertIn("What do you need to prepare for this listing?", HTML)
        self.assertIn("Start a property listing", HTML)
        self.assertIn("Set up tenant representation", HTML)
        ...
        self.assertIn("{ label: 'Start a property listing', copy: 'Begin with the seller, property, timing, and next steps.', primary: true, action: () => openListingWorkspace(true), isWorkspaceOpen: () => Boolean(document.getElementById('listingWorkspaceStartCard')) }", HTML)
        ...
        self.assertIn("What do you need to prepare for this tenant representation?", HTML)
