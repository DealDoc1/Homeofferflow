...
class AgentActivationDashboardTests(unittest.TestCase):
    ...
    def test_first_offer_activation_action_reveals_the_four_transaction_choices_without_creating_a_draft(self):
        ...
        self.assertIn("localStorage.getItem('hof_agent_workflow_choice')", HTML)
        self.assertIn("sessionStorage.setItem('hof_agent_workflow_choice', kind)", HTML)
        self.assertIn("localStorage.setItem('hof_agent_workflow_choice', kind)", HTML)
        self.assertIn('You’re working on: ${labels[savedChoice]}.', HTML)
    ...
    def test_transaction_interview_recommends_a_package_before_opening_a_workspace(self):
        ...
        self.assertIn("window.hofOpenAgentPackageInterview = function hofOpenAgentPackageInterview(kind, options = {})", HTML)
        self.assertIn("What do you need for this purchase?", HTML)
        self.assertIn("What do you need for this listing?", HTML)
        self.assertIn("What do you need for this tenant?", HTML)
        self.assertNotIn("What do you want to do next?", HTML)
        self.assertIn("Write a purchase offer", HTML)
        self.assertIn("We’ll prepare the supported documents your answers require.", HTML)
        self.assertIn("Create a buyer representation agreement", HTML)
        self.assertIn("Start a listing workspace", HTML)
        self.assertIn("Create a tenant representation agreement", HTML)
        ...
        self.assertIn("{ label: 'Start a listing workspace', copy: 'Begin with the seller, property, timing, and next steps.', primary: true, action: () => openListingWorkspace(true), isWorkspaceOpen: () => Boolean(document.getElementById('listingWorkspaceStartCard')) }", HTML)
        self.assertNotIn("Prepare lease details", HTML)
        self.assertIn("const closeFollowUp = (reason = 'dismissed') => {", HTML)
        self.assertIn("returnFocus?.focus({ preventScroll: true });", HTML)
        self.assertIn("bindPackageQuestionKeys(followUp, () => closeFollowUp('escape'));", HTML)
        self.assertIn("bindPackageQuestionKeys(modal, () => closeInterview('escape'));", HTML)
    ...
    def test_relationship_workspace_uses_question_one_transaction_language(self):
        ...
        self.assertNotIn("Use this after choosing Buyer or Tenant representation above.", HTML)
    ...
    def test_lease_representation_choice_waits_for_its_explicit_approved_source_without_a_second_click(self):
        ...
        self.assertIn("Opening your selected document…", HTML)
    ...
    def test_customer_facing_workspace_labels_do_not_expose_internal_beta_language(self):
        ...
        self.assertNotIn("review beta readiness", HTML)
        self.assertNotIn("into beta testing", HTML)
        self.assertNotIn("harden the beta", HTML)
        self.assertNotIn("remain roadmap items", HTML)
