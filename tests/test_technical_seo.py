...
SEO_GUIDES = {
    ...
    "texas-agent-form-library.html": (
        "Texas Agent and Broker Workspace",
        "Texas Agent Form Library Guide",
        "https://www.homeofferflow.com/agents",
    ),
    "texas-seller-financing-guide.html": (
        "Texas Agent and Broker Workspace",
        "Texas Seller Financing Workflow Guide",
        "https://www.homeofferflow.com/agents",
    ),
...
class TechnicalSeoTests(unittest.TestCase):
    ...
    def test_listing_and_lease_guides_measure_guide_specific_agent_ctas(self):
        ...
        self.assertIn("medium === 'organic_content'", metrics)
        self.assertIn("return 'organic';", metrics)
    ...
    def test_sitemap_supplies_verified_lastmod_dates_for_indexable_pages(self):
        ...
        self.assertEqual(dict(entries)["https://www.homeofferflow.com/"], "2026-09-09")
        for path in ("/agents", "/ondemand", "/partners", "/texas-home-service-partner-guide", "/buyers", "/sellers", "/texas-fsbo-guide", "/texas-seller-offer-review", "/texas-agent-offer-workflow", "/texas-lease-offer-workflow", "/texas-listing-workflow", "/texas-agent-form-library", "/texas-seller-financing-guide", "/texas-buyer-representation-guide", "/texas-flat-fee-mls-guide", "/texas-seller-net-proceeds-calculator", "/texas-fsbo-closing-checklist", "/texas-homebuyer-offer-guide", "/texas-investor-offer-guide", "/investors", "/directory"):
            expected = "2026-09-08" if path in {"/texas-agent-offer-workflow", "/partners"} else "2026-09-07" if path in {"/agents", "/sellers"} else "2026-09-06" if path in {"/ondemand", "/investors"} else "2026-08-27" if path in {"/buyers", "/texas-seller-net-proceeds-calculator"} else "2026-08-24"
