...
class PublicDiscoveryMetadataTests(unittest.TestCase):
    def test_not_found_agent_routes_describe_the_same_question_two_package_choices(self):
        ...
        self.assertIn('Add the seller and property details. Keep disclosures and offer comparisons together.', NOT_FOUND)
        self.assertIn('Choose a purchase offer, buyer representation agreement, addendum, or showing form.', NOT_FOUND)
        self.assertIn('Add the landlord and property details to begin.', NOT_FOUND)
        self.assertIn('Choose the representation agreement that fits this tenant.', NOT_FOUND)
    ...
    def test_homebuyer_guide_keeps_mobile_metadata_inside_a_valid_document_head(self):
        self.assertTrue(BUYER_GUIDE.startswith('<!doctype html>\n<html lang="en"><head>'))
        head = BUYER_GUIDE.split("</head>", 1)[0]
    ...
    def test_crawlers_can_discover_the_public_marketing_routes(self):
        ...
        self.assertIn('https://www.homeofferflow.com/texas-flat-fee-mls-guide', SITEMAP)
    ...
    def test_partner_acquisition_page_has_share_metadata_and_a_direct_application_path(self):
        ...
        self.assertIn('From application to a live placement.', PARTNERS)
        self.assertIn('Apply with the essentials', PARTNERS)
        self.assertIn('Review secure checkout', PARTNERS)
        self.assertIn('Finish setup and e-sign', PARTNERS)
        self.assertIn('Partner Marketplace Agreement for e-signature', PARTNERS)
    ...
    def test_agent_offer_workflow_guide_is_a_crawlable_people_first_path_to_the_workspace(self):
        ...
        self.assertIn('Choose transaction type', AGENT_GUIDE)
        ...
        self.assertNotIn('href="/?agent=1&amp;utm_source=texas_agent_offer_workflow', AGENT_GUIDE)
    ...
    def test_homepage_exposes_the_shared_agent_form_library_path(self):
        ...
        self.assertIn('relationship documents', AGENT_GUIDE)
        self.assertIn('confirm recipients before sending it for signature', AGENT_GUIDE)
    ...
    def test_seller_acquisition_page_is_indexable_and_routes_to_the_existing_safe_intake(self):
        ...
        self.assertIn('Any paid service requires separate scope and pricing confirmation.', SELLERS)
    ...
    def test_agent_workspace_page_is_indexable_and_routes_to_passwordless_sign_in(self):
        ...
        self.assertIn('"name":"Can I prepare documents from the shared library?"', AGENTS)
        self.assertIn('"name":"Does this replace broker supervision?"', AGENTS)
        self.assertIn('guided relationship, consumer-notice, seller-disclosure, finance, environmental, and mineral documents', AGENTS)
        self.assertIn('TXR-1501, TXR-1506, TXR-1507, TXR-1508, TXR-1905, TXR-1914, TXR-1917, TXR-1919, TXR-1948, TXR-1953, and TXR-1954 document preparation for every signed-in agent', AGENTS)
        self.assertIn('Listings and seller work', AGENTS)
        self.assertIn('TREC-55-1 seller disclosures and optional TREC-61-0 water disclosures for review', AGENTS)
        self.assertIn('seller consultation brief, and compare incoming offers', AGENTS)
