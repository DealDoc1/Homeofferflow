...
class PublicDiscoveryMetadataTests(unittest.TestCase):
    ...
    def test_homebuyer_offer_page_is_indexable_and_routes_to_the_existing_safe_workflow(self):
        ...
        self.assertIn("const rawSource = String(source.get('utm_source') || '').trim().toLowerCase();", BUYERS)
        self.assertIn("const campaignChannel = allowedChannels.has(rawSource) ? rawSource", BUYERS)
        self.assertIn("rawSource === 'homeofferflow_admin' && allowedChannels.has(medium)", BUYERS)
    ...
    def test_agent_workspace_page_is_indexable_and_routes_to_passwordless_sign_in(self):
        ...
        self.assertIn('private TXR-1501, TXR-1506, TXR-1507, and TXR-1508 draft creation for every signed-in agent', AGENTS)
        self.assertIn('Private seller planning', AGENTS)
        self.assertIn('seller listing launch checklist or consultation brief', AGENTS)
        self.assertIn('The signature queue identifies the signer and requires an explicit recipient confirmation before sending.', AGENTS)
