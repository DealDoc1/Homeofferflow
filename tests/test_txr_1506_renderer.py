...
from api.txr_1506 import build_signwell_fields_txr1506, render_txr_1506
...
class Txr1506RendererTests(unittest.TestCase):
    ...
    def test_signer_map_supports_one_or_two_consumers_and_requires_plan(self):
        one = build_signwell_fields_txr1506({**sample_data(), "signer_plan": "consumers_only"}, client_count=1)[0]
        two = build_signwell_fields_txr1506(sample_data(), client_count=2)[0]
        self.assertEqual(len(one), 7)
