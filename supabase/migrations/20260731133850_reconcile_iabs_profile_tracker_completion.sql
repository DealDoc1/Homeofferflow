...
set status = 'production',
    ...
    qa_status = 'passed',
    current_release = 'Production agent IABS profile attachment (2026-07-31)',
    known_issues = null,
    next_action = 'Monitor upload and optional-append errors; never attach an IABS automatically.',
    is_locked = true,
...
insert into public.hof_roadmap_items
  (slug, category, title, description, priority, status, environment,
   ...
   github_ref, is_locked, metadata, completed_at)
values
  (
    'agent-iabs-profile',
    'Agent Workspace',
...
on conflict (slug) do update set
  ...
  completed_at = coalesce(public.hof_roadmap_items.completed_at, excluded.completed_at);
