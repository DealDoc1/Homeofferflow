-- Persist the individual agent attestation required at point of use for restricted Texas REALTORS / NAR member-form workflows.
begin;
...
set
  ...
  next_action = 'Run authenticated agent point-of-use QA for one restricted-form draft. Do not activate signing until source-owner approval, signer-plan review, rendered-PDF QA, completed-signature QA, and product release authority are recorded.',
  updated_at = now()
where slug in (
