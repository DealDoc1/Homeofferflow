"""Render private financing-addendum QA specimens without sending documents."""
import argparse
...
from tests.test_financing_addendum_layout import sample
...
def main():
    ...
    cases={kind:sample(financing=kind) for kind in ('conventional','fha','va','usda')}
    cases['no-approval-one-buyer']=sample(buyer2Email='',buyerApproval='no',originationCap=0)
    cases['long']=sample(financing='fha',address='A'*400,
                         fhaSection='José 李 <b>Section</b> & '+'Z'*5000)
    for label,offer in cases.items():
        ...
        selected=sorted({f['page'] for f in fields if 'financing' in f['api_id']})
