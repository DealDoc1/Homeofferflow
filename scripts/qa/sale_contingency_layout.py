"""Local TREC 10-6 source-backed field review; no signing or email activity."""
import argparse
...
from tests.test_sale_contingency_layout import sample
...
def main():
    ...
    cases={'ordinary':sample(), 'zero-one-buyer':sample(buyer2Email='',saleAdditionalEarnest=0),
           'long':sample(address='A'*400,salePropertyAddr='José 李 <b>Old</b> & '+'Z'*5000)}
    for label,offer in cases.items():
        ...
        selected=sorted({f['page'] for f in fields if 'sale_other_property' in f['api_id'] or f['api_id'].startswith('sale_continuation_')})
