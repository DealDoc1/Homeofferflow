...
def _parse_txr_1948_draft(data):
    ...
    The interview records only the printed choice and agent-entered values. It
    does not assess value, recommend a waiver, or create a signature request.
    """
    if data.get("formCode") != TXR_1948_FORM_CODE:
        raise ValueError("Only TXR-1948 is available through this action.")
    form_source_id = _agreement_text(data.get("formSourceId"), "Approved TXR-1948 source", 80)
    try:
        form_source_id = str(uuid.UUID(form_source_id))
    except (TypeError, ValueError, AttributeError):
        raise ValueError("Choose an available TXR-1948 source from the HomeOfferFlow library.")
    ...
    if len({name.casefold() for name in all_names}) != len(all_names):
        raise ValueError("List each buyer or seller only once.")
    choice = str(data.get("appraisalChoice") or "").strip()
    if choice not in {"waiver", "partial_waiver", "additional_right"}:
        raise ValueError("Choose one appraisal-review option printed on TXR-1948.")
    ...
    partial_value = _agreement_money(data.get("partialValue"), "Partial-waiver appraisal value")
    additional_days = whole_number(data.get("additionalDays"), "Additional termination time") if choice == "additional_right" else ""
    additional_value = _agreement_money(data.get("additionalValue"), "Additional-right appraisal value") if choice == "additional_right" else ""
    if choice == "partial_waiver" and not partial_value:
        raise ValueError("Enter the partial-waiver appraisal value.")
    if choice == "additional_right" and (not additional_days or not additional_value):
        raise ValueError("Enter the additional termination time and appraisal value.")
    if data.get("appraisalReviewAcknowledgment") is not True:
        raise ValueError("Confirm that the parties will review the appraisal choice before signing.")
    return {
        "form_source_id": form_source_id,
        "client_names": all_names,
        "agreement_data": {
            ...
            "buyer_names": buyer_names,
            "seller_names": seller_names,
            "appraisal_choice": choice,
            "partial_value": partial_value,
            "additional_days": additional_days,
            "additional_value": additional_value,
            "appraisal_review_acknowledgment": True,
        },
    }
...
def _parse_txr_1954_draft(data):
    """Validate a private TXR-1954 fixture-lease review draft."""
    if data.get("formCode") != TXR_1954_FORM_CODE:
        raise ValueError("Only TXR-1954 is available through this action.")
    form_source_id = _agreement_text(data.get("formSourceId"), "Approved TXR-1954 source", 80)
    try:
        form_source_id = str(uuid.UUID(form_source_id))
    except (TypeError, ValueError, AttributeError):
        raise ValueError("Choose an available TXR-1954 source from the HomeOfferFlow library.")
    ...
    def choices(key, label, allowed):
        values = data.get(key)
        if not isinstance(values, list) or not values or len(values) > len(allowed):
            raise ValueError(f"Choose at least one {label}.")
        clean = [_agreement_text(value, label, 40) for value in values]
        if len(set(clean)) != len(clean) or any(value not in allowed for value in clean):
            raise ValueError(f"Choose only the printed {label} options.")
        return clean
    ...
    if len({name.casefold() for name in all_names}) != len(all_names):
        raise ValueError("List each buyer or seller only once.")
    leased_fixture_types = choices("leasedFixtureTypes", "leased fixture", {"solar_panels", "propane_tanks", "water_softener", "security_system", "other"})
    assumed_fixture_leases = choices("assumedFixtureLeases", "assumed fixture lease", {"solar_panels", "propane_tanks", "water_softener", "security_system", "other"})
    leased_other = _agreement_text(data.get("leasedFixturesOther"), "Other leased fixture", 180) if "other" in leased_fixture_types else ""
    assumed_other = _agreement_text(data.get("assumedFixtureLeasesOther"), "Other assumed fixture lease", 180) if "other" in assumed_fixture_leases else ""
    buyer_first_cost = _agreement_money(data.get("buyerFirstCost"), "Buyer first fixture-lease cost")
    removal_choice = str(data.get("removalChoice") or "").strip()
    if removal_choice not in {"will", "will_not"}:
        raise ValueError("Choose whether Seller will remove unassumed leased fixtures.")
    delivery_choice = str(data.get("deliveryChoice") or "").strip()
    if delivery_choice not in {"received", "not_received", "oral_notice"}:
        raise ValueError("Choose the fixture-lease delivery option.")
    oral_notice = _agreement_text(data.get("oralFixtureLeaseNotice"), "Oral fixture-lease notice", 500) if delivery_choice == "oral_notice" else ""
    if data.get("fixtureLeaseReviewAcknowledgment") is not True:
        raise ValueError("Confirm that the parties will review fixture leases with the appropriate professionals before signing.")
    return {
        "form_source_id": form_source_id,
        "client_names": all_names,
        "agreement_data": {
            ...
            "buyer_names": buyer_names,
            "seller_names": seller_names,
            "leased_fixture_types": leased_fixture_types,
            "leased_fixtures_other": leased_other,
            "assumed_fixture_leases": assumed_fixture_leases,
            "assumed_fixture_leases_other": assumed_other,
            "buyer_first_cost": buyer_first_cost,
            "removal_choice": removal_choice,
            "delivery_choice": delivery_choice,
            "oral_fixture_lease_notice": oral_notice,
            "fixture_lease_review_acknowledgment": True,
        },
    }
...
def _parse_txr_1914_draft(data):
    ...
    The source warns that seller financing can be complex. This records only
    agent-entered source-form choices for a private review PDF; it neither
    recommends terms nor creates a promissory note, deed of trust, or signing
    request.
    """
    if data.get("formCode") != TXR_1914_FORM_CODE:
        raise ValueError("Only TXR-1914 is available through this action.")
    form_source_id = _agreement_text(data.get("formSourceId"), "Approved TXR-1914 source", 80)
    try:
        form_source_id = str(uuid.UUID(form_source_id))
    except (TypeError, ValueError, AttributeError):
        raise ValueError("Choose an available TXR-1914 source from the HomeOfferFlow library.")
...
async def _create_txr_1948_draft(user, data):
...
async def _create_txr_1954_draft(user, data):
    return await _create_representation_draft(user, data, TXR_1954_FORM_CODE, _parse_txr_1954_draft)
...
def _signwell_document_matches_signing_request(document, expected_fields, expected_recipients):
    from lib.signwell_request import document_matches_signing_request
    return document_matches_signing_request(document, expected_fields, expected_recipients)
...
def _signwell_send_options(payload):
    from lib.signwell_request import send_options
    return send_options(payload)
        ...
        try:
            ...
            for item in partner_landing_events:
                if item.get("event_type") not in {"partner_landing_viewed", "partner_landing_cta_selected"}:
                    continue
                channel = str((item.get("metadata") or {}).get("channel") or "direct").strip().lower()
                if channel in {"direct", "organic", "pwa_shortcut", "email", "social", "referral", "other"}:
                    partner_landing_channel_counts[channel] = partner_landing_channel_counts.get(channel, 0) + 1
            partner_landing_channel_counts = dict(sorted(
                partner_landing_channel_counts.items(), key=lambda item: (-item[1], item[0])
            ))
            ...
            for item in events:
                ...
                if str(metadata.get("directorySurface") or "").strip() == "pwa_provider_directory":
                    if event_type == "partner_directory_impression":
            ...
            pwa_authenticated_shortcut_actions = (
                "workspace", "brokerage_setup", "transaction_start", "listing_tools", "relationship_drafts", "offer_review", "new_offer", "signing_queue", "attention_queue",
            )
            ...
            homebuyer_landing_channels = (
                "direct", "homepage", "pwa_shortcut", "direct_outreach", "email", "social", "referral", "local_event", "print", "organic", "unspecified",
            )
            ...
            agent_landing_cta_paths = ("client_draft", "seller_listing", "lease_listing", "relationship_drafts", "lease_representation", "listing_guide", "lease_guide", "form_library_guide")
            ...
            agent_private_review_next_step_clicked_count = len([
                item for item in events
            ...
            seller_intake_event_types = {
                ...
                "fsbo_seller_plan_downloaded", "fsbo_seller_plan_copied", "fsbo_seller_plan_printed", "fsbo_seller_plan_shared", "fsbo_seller_support_contact_opened",
            }
            ...
            metrics = {
                ...
                "sellerPlanPrintedCount": seller_intake_event_counts["fsbo_seller_plan_printed"],
                "sellerPlanSharedCount": seller_intake_event_counts["fsbo_seller_plan_shared"],
    ...
    def do_POST(self):
        ...
        try:
            ...
            if data.get("action") == "create_txr_1953_draft":
                ...
                _json(self, 201, {"status": "ok", "agreement": draft})
                return
            if data.get("action") == "create_txr_1954_draft":
                draft = asyncio.run(_create_txr_1954_draft(user, data))
                _json(self, 201, {"status": "ok", "agreement": draft})
                return
