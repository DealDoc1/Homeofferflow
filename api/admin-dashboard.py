...
async def _render_seller_disclosure_draft_preview(user, draft_id, review_context=None):
    """Render an agent-owned or token-authorized seller draft as a private preview.

    Review links may only render an unsigned preview. This function never signs,
    sends, activates, or changes a draft.
    """
    try:
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            import asyncio
            query = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
            attest_token = str((query.get("attest_seller_review") or [""])[0]).strip()
            if attest_token:
                ...
                result = asyncio.run(_attest_seller_review(attest_token, data))
