...
# SignWell request/response payloads can contain customer names and email
# addresses. Keep verbose diagnostics opt-in rather than emitting them for
# every production offer.
...
def append_uploaded_disclosure_docs(merger, offer):
    """
    Uploaded-doc workflow:
    - Accepts offer["uploadedDisclosureDocs"] as a list of PDF base64 payloads.
    - Appends valid PDFs after the generated TREC packet/addenda.
    - Manual SignWell field placement is handled in build_signwell_fields().
    """
    docs = offer.get("uploadedDisclosureDocs") or offer.get("uploadedDocs") or []
    ...
    for idx, doc in enumerate(docs[:max_docs]):
        if not isinstance(doc, dict):
            continue
        name = str(doc.get("name") or f"uploaded_doc_{idx + 1}.pdf")
        b64 = doc.get("base64") or doc.get("file_base64") or doc.get("data") or ""
        if not b64:
            continue
        try:
            ...
            if not raw.startswith(b"%PDF"):
                print("UPLOAD DOC SKIP not pdf:", name)
                continue
            if len(raw) > max_bytes:
                print("UPLOAD DOC SKIP too large:", name, len(raw))
                continue
            reader = PdfReader(BytesIO(raw))
            if len(reader.pages) < 1:
                print("UPLOAD DOC SKIP no pages:", name)
                continue
            merger.append(reader)
            appended += 1
        except Exception as e:
            print("UPLOAD DOC SKIP error:", name, str(e))
            continue
    return appended
...
def fill_and_merge(offer):
    ...
    # Phase 1 uploaded docs: append agent-provided PDF disclosures after generated TREC forms/addenda.
    append_uploaded_disclosure_docs(merger, s)
...
def get_uploaded_disclosure_docs_with_page_counts(offer):
    """
    Return uploaded PDF docs with safe page counts for appended-document SignWell placement.
    Does not persist or mutate required data beyond local counting.
    """
    docs = offer.get("uploadedDisclosureDocs") or offer.get("uploadedDocs") or []
    if not isinstance(docs, list):
        return []
    ...
    for idx, doc in enumerate(docs[:max_docs]):
        ...
        try:
            ...
            copied = dict(doc)
            copied["name"] = name
            copied["page_count"] = page_count
            copied["pageCount"] = page_count
            out.append(copied)
        except Exception as e:
            print("UPLOAD DOC COUNT SKIP:", name, str(e))
            continue
    return out
...
def build_signwell_fields(offer, pdf_bytes):
    ...
    # Live QA: both buyer SignWell fields were still too low on TREC 57-0; move them materially up onto the Buyer lines.
    if non_realty_page:
        add_sig_date_pair("buyer1_non_realty_items_addendum", non_realty_page, 76, 700, 246, 700, "1")
        if has_buyer2:
            add_sig_date_pair("buyer2_non_realty_items_addendum", non_realty_page, 76, 774, 246, 774, "2")
    ...
    # Uploaded disclosure docs: manual buyer-side SignWell placement.
    # Frontend stores click coordinates as SignWell's 816x1056-style coordinate space.
    uploaded_docs = get_uploaded_disclosure_docs_with_page_counts(offer)
    uploaded_start_page = next_page
    uploaded_page_cursor = uploaded_start_page

    placement_type_map = {
        "buyer1_signature": {"recipient_id": "1", "field_type": "signature", "width": 145, "height": 20},
        "buyer1_date": {"recipient_id": "1", "field_type": "date", "width": 66, "height": 16, "date_format": "MM/DD/YYYY", "lock_sign_date": True},
        "buyer1_initials": {"recipient_id": "1", "field_type": "initials", "width": 24, "height": 10},
        "buyer2_signature": {"recipient_id": "2", "field_type": "signature", "width": 145, "height": 20},
        "buyer2_date": {"recipient_id": "2", "field_type": "date", "width": 66, "height": 16, "date_format": "MM/DD/YYYY", "lock_sign_date": True},
        "buyer2_initials": {"recipient_id": "2", "field_type": "initials", "width": 24, "height": 10},
    }

    for doc_idx, doc in enumerate(uploaded_docs):
        doc_page_count = int(doc.get("page_count") or doc.get("pageCount") or 1)
        ...
        if isinstance(placements, list):
            for place_idx, placement in enumerate(placements):
                ...
                spec = placement_type_map.get(placement_type)
                if not spec:
                    continue
                recipient_id = spec.get("recipient_id", "1")
                if recipient_id == "2" and not has_buyer2:
                    continue

                try:
                    page_in_doc = max(1, int(float(placement.get("page") or 1)))
                except Exception:
                    page_in_doc = 1
                if page_in_doc > doc_page_count:
                    page_in_doc = doc_page_count
                ...
                extra = {}
                if spec.get("date_format"):
                    extra["date_format"] = spec["date_format"]
                if spec.get("lock_sign_date"):
                    extra["lock_sign_date"] = True

                safe_doc_name = str(doc.get("name") or f"uploaded_doc_{doc_idx + 1}")[:32]
                safe_doc_name = "".join(ch if ch.isalnum() else "_" for ch in safe_doc_name).strip("_") or f"uploaded_doc_{doc_idx + 1}"
                add_field(
                    f"uploaded_{doc_idx + 1}_{safe_doc_name}_p{page_in_doc}_{placement_type}_{place_idx + 1}",
                    spec.get("field_type"),
                    absolute_page,
                    x,
                    y,
                    ...
                    height=spec.get("height", 22),
                    **extra
                )
        uploaded_page_cursor += doc_page_count
...
def create_signwell_signature_request(offer, pdf_bytes):
    ...
    if str(offer.get("userType") or offer.get("role") or "homebuyer").lower() in ["agent", "investor"] and (agent_email or agent_phone):
        signwell_message = (
            ...
            "Disclaimer: https://www.homeofferflow.com/disclaimer.html\n\n"
            "Important: HomeOfferFlow is a form-completion software tool. It is not a law firm, does not provide legal advice, "
            "and does not represent any party in the transaction. If you need legal, brokerage, financing, inspection, or negotiation advice, "
            "consult an appropriately licensed professional before signing."
        )
    else:
        signwell_message = (
            prepared_by_line +
...
def extract_signwell_document_id(signwell_info):
    if not isinstance(signwell_info, dict):
        return ""
    return (
        signwell_info.get("document_id")
        or signwell_info.get("response", {}).get("id")
        or signwell_info.get("response", {}).get("document_id")
        or ""
    )
...
def save_generated_offer_to_supabase(offer, customer_email="", signwell_info=None, subscription_user_id=None):
    ...
    try:
        ...
        buyer_first = first_present(offer.get("buyer1First"), offer.get("firstName"), "")
        buyer_last = first_present(offer.get("buyer1Last"), offer.get("lastName"), "")
        buyer_name = first_present(
            offer.get("buyer1"),
            offer.get("buyerName"),
            f"{buyer_first} {buyer_last}".strip(),
            "Buyer"
        )
        buyer_email = first_present(offer.get("buyerEmail"), offer.get("email"), customer_email, "")
        property_address = first_present(offer.get("address"), offer.get("propAddress"), "Untitled offer")
        offer_price = money_number(first_present(offer.get("price"), offer.get("offerPrice")))

        signwell_document_id = extract_signwell_document_id(signwell_info)
        signwell_status = ""
        if isinstance(signwell_info, dict):
            signwell_status = first_present(
                signwell_info.get("response", {}).get("status"),
                signwell_info.get("status"),
        ...
        payload = {
            ...
            "property_city": first_present(offer.get("city"), offer.get("propCity"), "") or None,
            "property_state": first_present(offer.get("state"), offer.get("propState"), "TX") or None,
            "property_zip": first_present(offer.get("zip"), offer.get("propZip"), "") or None,
            "property_county": first_present(offer.get("county"), offer.get("propCounty"), "") or None,
            "buyer_name": buyer_name,
            "buyer_email": buyer_email or None,
            "buyer_phone": first_present(offer.get("buyerPhone"), offer.get("phone"), "") or None,
            "buyer2_name": first_present(offer.get("buyer2"), offer.get("buyer2Name"), "") or None,
            "buyer2_email": first_present(offer.get("buyer2Email"), "") or None,
            "seller_name": first_present(offer.get("seller"), offer.get("seller1"), offer.get("sellerName"), "") or None,
            "agent_name": first_present(offer.get("agentName"), offer.get("agent_name"), "") or None,
            "agent_email": first_present(offer.get("agentEmail"), offer.get("agent_email"), "") or None,
            "agent_phone": first_present(offer.get("agentPhone"), offer.get("agent_phone"), "") or None,
            "agent_license": first_present(offer.get("agentLicense"), "") or None,
            "agent_brokerage": first_present(offer.get("agentBrokerage"), "") or None,
            "agent_broker_license": first_present(offer.get("agentBrokerLicense"), "") or None,
            "offer_price": offer_price,
            "financing_type": first_present(offer.get("financing"), offer.get("financingType"), "") or None,
            "loan_amount": money_number(offer.get("loanAmount")),
            "down_payment": money_number(offer.get("downPayment")),
            "earnest_money": money_number(first_present(offer.get("earnest"), offer.get("earnestMoney"))),
            "option_fee": money_number(offer.get("optionFee")),
            "option_days": int(float(str(offer.get("optionDays")))) if str(offer.get("optionDays", "")).strip().replace(".", "", 1).isdigit() else None,
            "closing_date": first_present(offer.get("closingDate"), "") or None,
            "title_company": first_present(offer.get("titleCompany"), "") or None,
        ...
        try:
            ...
            if isinstance(data, list) and data:
                offer_id = data[0].get("id")
                if offer_id:
                    offer["_hofOfferId"] = offer_id
                return offer_id
        except Exception:
            return None
    except Exception as e:
...
def send_admin_order_email(offer, customer_email="", signwell_info=None):
    ...
    buyer_name = first_present(offer.get("buyer1"), offer.get("buyerName"), "Buyer")
    buyer2_name = first_present(offer.get("buyer2"), offer.get("buyer2Name"), "")
    buyer_email = first_present(offer.get("buyerEmail"), customer_email, "")
    buyer2_email = first_present(offer.get("buyer2Email"), "")
    addr = first_present(offer.get("address"), offer.get("propAddress"), "Property")
    city = first_present(offer.get("city"), offer.get("propertyCity"), "")
    county = first_present(offer.get("county"), offer.get("propertyCounty"), "")
    price = fmt_money(first_present(offer.get("price"), offer.get("offerPrice"), ""))
    plan = first_present(offer.get("_plan"), offer.get("selectedPlan"), offer.get("plan"), "")
    financing = first_present(offer.get("financing"), offer.get("financingType"), "")
    closing_date = first_present(offer.get("closingDate"), "")
    earnest = fmt_money(first_present(offer.get("earnest"), offer.get("earnestMoney"), ""))
    option_fee = fmt_money(first_present(offer.get("optionFee"), ""))
    option_days = first_present(offer.get("optionDays"), "")
    ...
    html = f"""
      <div style="font-family:Arial,sans-serif;max-width:760px;margin:0 auto;">
        <h2 style="color:#1a2f4a;">New HomeOfferFlow Order</h2>
        <p>A paid offer packet was generated.</p>

        <h3>Buyer</h3>
        <table style="border-collapse:collapse;width:100%;margin-top:0.5rem;">
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Buyer 1</strong></td><td style="padding:8px;border:1px solid #ddd;">{buyer_name}</td></tr>
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Buyer 1 Email</strong></td><td style="padding:8px;border:1px solid #ddd;">{buyer_email}</td></tr>
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Buyer 2</strong></td><td style="padding:8px;border:1px solid #ddd;">{buyer2_name or "—"}</td></tr>
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Buyer 2 Email</strong></td><td style="padding:8px;border:1px solid #ddd;">{buyer2_email or "—"}</td></tr>
        </table>
        ...
        <table style="border-collapse:collapse;width:100%;margin-top:0.5rem;">
          ...
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>City / County</strong></td><td style="padding:8px;border:1px solid #ddd;">{city or "—"} / {county or "—"}</td></tr>
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Offer Price</strong></td><td style="padding:8px;border:1px solid #ddd;">${price or "—"}</td></tr>
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Plan</strong></td><td style="padding:8px;border:1px solid #ddd;">{plan or "self"}</td></tr>
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Financing</strong></td><td style="padding:8px;border:1px solid #ddd;">{financing or "—"}</td></tr>
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Closing Date</strong></td><td style="padding:8px;border:1px solid #ddd;">{closing_date or "—"}</td></tr>
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Earnest / Option</strong></td><td style="padding:8px;border:1px solid #ddd;">${earnest or "—"} / ${option_fee or "—"} for {option_days or "—"} days</td></tr>
        </table>

        <h3>SignWell</h3>
        <table style="border-collapse:collapse;width:100%;margin-top:0.5rem;">
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Status</strong></td><td style="padding:8px;border:1px solid #ddd;">{signwell_status}</td></tr>
          <tr><td style="padding:8px;border:1px solid #ddd;"><strong>Document ID</strong></td><td style="padding:8px;border:1px solid #ddd;">{signwell_doc_id or "—"}</td></tr>
        </table>

        <p style="margin-top:1rem;background:#fff3cd;padding:1rem;border-radius:8px;font-size:0.9rem;">
          Internal alert only. Buyer-facing PDF email was sent separately.
        </p>
      </div>
    """
