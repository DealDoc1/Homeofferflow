...
def fill_and_merge(offer):
    ...
    if has_appraisal and os.path.exists(APPRAISAL_PDF):
        appraisal_choice = str(s.get("appraisalAddendum") or "none").strip().lower()
        appraisal_pages = {
            0: [
                (224, 655, addr_full, 8),
                (51, 550, ck(appraisal_choice == "waiver"), "check_small"),
                (51, 470, ck(appraisal_choice == "partial"), "check_small"),
                (226, 405, fmt_money(s.get("appraisalPartialValue", "")) if appraisal_choice == "partial" else ""),
                (51, 348, ck(appraisal_choice == "additional"), "check_small"),
                (64, 324, str(first_present(s.get("appraisalTerminateDays"), "7")) if appraisal_choice == "additional" else ""),
                (119, 289, fmt_money(s.get("appraisalTerminateValue", "")) if appraisal_choice == "additional" else ""),
            ],
        }
        appraisal_pages = add_debug_grid_to_pages(appraisal_pages)
        merger.append(PdfReader(BytesIO(stamp_pdf(APPRAISAL_PDF, appraisal_pages))))
...
def build_signwell_fields(offer, pdf_bytes):
    ...
    if appraisal_page:
        add_field("buyer1_appraisal_addendum_signature", "signature", appraisal_page, 85, 804, recipient_id="1", width=170, height=22)
        if has_buyer2:
            add_field("buyer2_appraisal_addendum_signature", "signature", appraisal_page, 85, 892, recipient_id="2", width=170, height=22)
