...
def build_pages_data(
    ...
    pages[0] = [
        ...
        (48, 194, ck(lease_residential), "check_small"),
        (48, 169, ck(lease_fixture), "check_small"),
        (48, 133, ck(lease_natural), "check_small"),
        (61,  97, ck(lease_natural and val_lower(lease_nr_delivered) == "yes"), "check_small"),
        (61,  83, ck(lease_natural and val_lower(lease_nr_delivered) == "no"), "check_small"),
        (363, 83, str(lease_nr_days) if lease_natural and val_lower(lease_nr_delivered) == "no" else ""),
        (237, 70, str(lease_nr_term_days) if lease_natural and val_lower(lease_nr_delivered) == "no" else ""),
    ]
    ...
    pages[1] = [
        ...
        (278, 641, fmt_money(first_present(s.get("additionalEarnest"), s.get("additionalEarnestMoney")))),
        (405, 641, str(first_present(s.get("additionalEarnestDays"), s.get("additionalEarnestDeadlineDays"))) if first_present(s.get("additionalEarnest"), s.get("additionalEarnestMoney")) else ""),
    ...
    pages[3] = [
        ...
        (71, 286, s.get("requiredNotices", ""), 7),
    ...
    if has_loan and os.path.exists(FINANCING_PDF):
        ...
        fin_pages = {
            0: [
                ...
                (58,  420, ck(financing == "fha"), "check_small"),
                (175, 421, first_present(s.get("fhaSection"), s.get("fhaProgram"), "203(b)") if financing == "fha" else "", 8),
                (91,  407, fmt_money(s.get("loanAmount", "")) if financing == "fha" else ""),
                (114, 393, loan_years if financing == "fha" else ""),
                (361, 393, interest_cap if financing == "fha" else ""),
                (116, 382, interest_first_years if financing == "fha" else ""),
                (247, 373, origination_cap if financing == "fha" else ""),

                # D. VA guaranteed financing.
                (58,  363, ck(financing == "va"), "check_small"),
                (440, 363, fmt_money(s.get("loanAmount", "")) if financing == "va" else ""),
                (479, 349, loan_years if financing == "va" else ""),
                (257, 338, interest_cap if financing == "va" else ""),
                (399, 338, interest_first_years if financing == "va" else ""),
                (107, 320, origination_cap if financing == "va" else ""),
