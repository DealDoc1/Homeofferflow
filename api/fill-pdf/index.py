...
def build_signwell_fields(offer, pdf_bytes):
    """
    Build SignWell field placement for the single merged HomeOfferFlow PDF.

    SignWell expects fields as a 2D array: one inner array per uploaded file.
    Since HomeOfferFlow uploads one merged PDF, we return [fields_for_file_1].

    Coordinate note:
    These are first-pass test-mode positions. They are intentionally centralized
    in this helper so we can nudge them after a real SignWell test document.
    """
    try:
    ...
    has_buyer2 = bool(buyer2_email)

    fields = []

    def add_field(api_id, field_type, page, x, y, recipient_id="1", width=None, height=None, required=True, **extra):
        f = {
            "api_id": api_id,
            "type": field_type,
            "page": int(page),
    ...
    # Initial every page of the merged packet.
    # Buyer 1 initials near lower-right. Buyer 2, when present, just to the left.
    for page in range(1, page_count + 1):
        add_field(
            api_id=f"buyer1_initials_p{page}",
            field_type="initials",
            page=page,
            x=520,
            y=735,
        ...
        if has_buyer2:
            add_field(
                api_id=f"buyer2_initials_p{page}",
                field_type="initials",
                page=page,
                x=468,
                y=735,
    ...
    # Main TREC 1-4 Family contract signature page.
    # The merged contract starts with the 10-page main contract; page 10 is the main signature page.
    # These are first-pass fields and should be nudged after SignWell test-mode visual review.
    main_signature_page = 10 if page_count >= 10 else page_count

    add_field(
        api_id="buyer1_signature_main_contract",
        field_type="signature",
        page=main_signature_page,
        x=72,
        y=610,
        recipient_id="1",
        width=210,
    ...
    add_field(
        api_id="buyer1_date_main_contract",
        field_type="date",
        page=main_signature_page,
        x=380,
        y=610,
    ...
    if has_buyer2:
        add_field(
            api_id="buyer2_signature_main_contract",
            field_type="signature",
            page=main_signature_page,
            x=72,
            y=652,
        ...
        add_field(
            api_id="buyer2_date_main_contract",
            field_type="date",
            page=main_signature_page,
            x=380,
            y=652,
...
def post_signwell_document(payload):
    ...
    if r.status_code not in [200, 201, 202]:
        return False, {"status_code": r.status_code, "error": r.text[:500]}
...
def create_signwell_signature_request(offer, pdf_bytes):
    ...
    - Adds Buyer 1 signature/date.
    - Adds Buyer 2 signature/date when buyer2Email exists.
    - Adds initials for required buyers on every page of the merged packet.
    - If coordinate fields are rejected by SignWell, safely falls back to a
      signature-page request so the paid order still gets a signing email.
    """
    ...
    buyer2_email = first_present(offer.get("buyer2Email"), "")
    buyer2_name = first_present(offer.get("buyer2"), "Co-Buyer")
    ...
    base_payload = {
        ...
        "subject": f"Signature requested: HomeOfferFlow Offer — {addr}",
        "message": "Please review and sign your HomeOfferFlow offer packet. HomeOfferFlow is not a law firm and does not provide legal advice.",
    ...
    try:
        ...
        coordinate_payload = dict(base_payload)
        coordinate_payload["with_signature_page"] = False
        coordinate_payload["fields"] = fields

        ok, data = post_signwell_document(coordinate_payload)

        if ok:
            return {
                "enabled": True,
                "ok": True,
                "mode": "coordinate_fields",
                "test_mode": SIGNWELL_TEST_MODE,
        ...
        # Safe fallback: still send a SignWell request with the built-in signature page.
        fallback_payload = dict(base_payload)
        fallback_payload["with_signature_page"] = True

        fallback_ok, fallback_data = post_signwell_document(fallback_payload)

        if fallback_ok:
            return {
                "enabled": True,
                "ok": True,
                "mode": "fallback_signature_page",
                "test_mode": SIGNWELL_TEST_MODE,
                "coordinate_error": data,
                "document_id": fallback_data.get("id") or fallback_data.get("document_id"),
        ...
        return {
            "enabled": True,
            "ok": False,
            "mode": "failed_coordinate_and_fallback",
            "coordinate_error": data,
            "fallback_error": fallback_data
        }
