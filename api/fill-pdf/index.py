    ...
    if has_loan and os.path.exists(FINANCING_PDF):
        ...
        fin_pages = {
            0: [
                ...
                # C. FHA insured financing - fixed after regression test; prior coords landed on Texas Veterans row.
                (59,  421, ck(financing == "fha"), "check_small"),
                (90,  407, fmt_money(s.get("loanAmount", "")) if financing == "fha" else ""),
                (113, 392, loan_years if financing == "fha" else ""),
                (361, 392, interest_cap if financing == "fha" else ""),
                (116, 381, interest_first_years if financing == "fha" else ""),
                (246, 372, origination_cap if financing == "fha" else ""),

                # D. VA guaranteed financing - fixed after regression test.
                (59,  363, ck(financing == "va"), "check_small"),
                (443, 363, fmt_money(s.get("loanAmount", "")) if financing == "va" else ""),
                (479, 349, loan_years if financing == "va" else ""),
                (257, 337, interest_cap if financing == "va" else ""),
                (399, 337, interest_first_years if financing == "va" else ""),
                (106, 319, origination_cap if financing == "va" else ""),

                # E. USDA guaranteed financing - aligned to USDA row.
                (59,  307, ck(financing == "usda"), "check_small"),
                (482, 307, fmt_money(s.get("loanAmount", "")) if financing == "usda" else ""),
                (492, 293, loan_years if financing == "usda" else ""),
                (272, 281, interest_cap if financing == "usda" else ""),
                (428, 281, interest_first_years if financing == "usda" else ""),
                (139, 263, origination_cap if financing == "usda" else ""),
            ],
            1: [
                (205, 729, addr_full, 8),
