...
def build_signwell_fields(offer, pdf_bytes):
    ...
    - Buyer 1 initials on main contract pages 1-11
    - Buyer 2 signature/date/initials only when buyer2Email exists
    - Third Party Financing Addendum initials + signatures only when financing is not cash

    Still does NOT include seller fields and does NOT use the Executed/Effective Date box.
    """
    try:
    ...
    main_contract_pages = list(range(1, min(page_count, 11) + 1))
    ...
    # Main contract initials: header line on each TREC contract page.
    # These are first-pass coordinates and may need one visual nudge, but bundled to reduce deploy cycles.
    ...
    if has_financing_addendum and page_count >= 13:
        ...
        # Adjusted after SignWell visual QA: prior y=612/614 landed in Paragraph 4 text.
        add_field(
