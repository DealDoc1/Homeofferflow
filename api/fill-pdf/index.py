...
def create_signwell_signature_request(offer, pdf_bytes):
    """
    SignWell scaffold for HomeOfferFlow.

    Safe V1 behavior:
    - OFF by default unless SIGNWELL_ENABLED=true.
    - Uses SignWell test mode by default.
    - Uses SignWell signature page first so we can prove the pipeline safely.
    - Later we will replace with explicit signature/date/initial coordinates.
    """
    ...
    payload = {
        ...
        "files": [{"name": filename, "file_base64": base64.b64encode(pdf_bytes).decode()}]
    }
    ...
    try:
        ...
        if r.status_code not in [200, 201, 202]:
            return {"enabled": True, "ok": False, "status_code": r.status_code, "error": r.text[:500]}
