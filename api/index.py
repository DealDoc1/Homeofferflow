...
def build_signwell_fields(offer, pdf_bytes):
    """
    Build a deliberately minimal SignWell field placement for the single merged
    HomeOfferFlow PDF.

    This debug version is intentionally small:
    - Buyer 1 signature only - final nudge
    - Main TREC contract signature page only
    - No date field yet
    - No initials yet
    - No Buyer 2 yet

    Goal: prove the SignWell field payload shape first. After SignWell accepts
    this minimal request, add date, initials, Buyer 2, and addendum fields back
    one layer at a time.
    """
    try:
    ...
    # Main TREC 1-4 Family contract is first in the merged packet; page 10 is
    # the signature page. SignWell API page numbering appears to be 1-based.
    main_signature_page = 9 if page_count >= 10 else max(page_count - 1, 1)

    fields = [[{
        "api_id": "buyer1_signature_main_contract_debug",
        "type": "signature",
...
def create_signwell_signature_request(offer, pdf_bytes):
    ...
    # Debug phase: one recipient only until the simplest payload succeeds.
    ...
    payload = {
        ...
        "metadata": {
            ...
            "debug_payload": "minimal_buyer1_signature_final_nudge"
        }
    }
    ...
    try:
        ...
        if ok:
            return {
                "enabled": True,
                "ok": True,
                "mode": "minimal_buyer1_signature_final_nudge",
                "test_mode": SIGNWELL_TEST_MODE,
        ...
        return {
            "enabled": True,
            "ok": False,
            "mode": "minimal_buyer1_signature_failed",
            "test_mode": SIGNWELL_TEST_MODE,
            "field_count": len(fields[0]) if fields else 0,
            "signwell_error": data
        }
