...
def build_signwell_fields(offer, pdf_bytes):
    """
    SignWell CALIBRATION build — not production signing.

    Purpose: stop guessing coordinates. This places a small set of visible buyer-side
    calibration fields on the pages we care about so we can identify exact SignWell x/y values.
    ...
    def add_field(api_id, field_type, page, x, y, recipient_id="1", width=60, height=18, required=False, **extra):
    ...
    # Keep one known-good required field so SignWell has a required action and creates/sends the doc.
    # This is the already-validated Buyer 1 main contract signature field.
    add_field(
        "CAL_REQUIRED_buyer1_signature_main_locked",
        "signature",
        9 if page_count >= 9 else max(page_count, 1),
        115,
        532,
        recipient_id="1",
        width=165,
    ...
    # Main contract page 1 bottom initials calibration.
    # Current y=778 was too high/body. These test the actual bottom initial line zone.
    for i, y in enumerate([900, 930, 960, 990], start=1):
        add_field(
            f"CAL_main_p1_initials_y{y}",
            "initials",
            1,
            246,
            y,
    ...
    # Main contract page 8 bottom initials calibration, because page 8 has heavy addenda text near bottom.
    if page_count >= 8:
        for i, y in enumerate([900, 930, 960, 990], start=1):
            add_field(
                f"CAL_main_p8_initials_y{y}",
                "initials",
                8,
                246,
                y,
    ...
    if has_financing_addendum and page_count >= 13:
        # Third Party Financing Addendum page 1 bottom initials calibration.
        for y in [900, 930, 960, 990]:
            add_field(
                f"CAL_financing_p1_initials_y{y}",
                "initials",
                12,
                318,
                y,
        ...
        # Third Party Financing Addendum page 2 buyer signature/date calibration.
        # Prior y=718 was still too high in the text body. Test lower rows.
        for y in [825, 860, 895, 930]:
            add_field(
                f"CAL_financing_p2_signature_y{y}",
                "signature",
                13,
                118,
                y,
    ...
    print("SIGNWELL CALIBRATION field payload:", json.dumps({
        "page_count": page_count,
        "has_financing_addendum": has_financing_addendum,
        "field_count": len(fields_for_file),
        "note": "Calibration build only. Do not judge as final production signing map.",
        "fields": fields,
    })[:8000])
