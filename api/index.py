...
def build_signwell_fields(offer, pdf_bytes):
    """
    Broader SignWell field map so we are not deploying one field at a time.

    Includes:
    - Buyer 1 signature + date on main TREC contract signature page
    - Buyer 1 initials on main contract pages 1-8 only, calibrated near bottom initial line
    - Buyer 2 signature/date/initials only when buyer2Email exists
    - Third Party Financing Addendum buyer-side initials + signatures only when financing is not cash

    Buyer-side only. No seller fields. No effective-date box. No generic SignWell signature page.
    """
    try:
    ...
    # SignWell accepted these as 1-based page numbers in testing.
    ...
    # Main contract signature page. Buyer 1 top line is visually validated.
    add_field(
        "buyer1_signature_main_contract",
        "signature",
        main_signature_page,
        115,
        532,
    ...
    # Main contract buyer initials: only pages 1-8. Page 9 is the execution/signature page; pages 10-11 do not have buyer initial lines.
    # Buyer-side only; no seller initials are added.
    # v8: x moved right so the field lands on the actual blank after "Buyer", not over "Initialed for identification".
    for page in main_contract_pages:
        add_field(
            f"buyer1_initials_main_p{page}",
            "initials",
            page,
            258,
            1018,
    ...
    if has_buyer2:
        # Buyer 2 lower buyer line on the main signature page.
        add_field(
            "buyer2_signature_main_contract",
            "signature",
            main_signature_page,
            115,
            632,
    ...
    # Addendum pages are appended after the 11-page main contract.
    # For this packet, Third Party Financing Addendum is pages 12-13.
    if has_financing_addendum and page_count >= 13:
        financing_page_1 = 12
        financing_signature_page = 13

        # Financing addendum page 1 bottom initial blanks.
        # v8: slightly smaller and centered on the buyer initial blank.
        add_field(
            "buyer1_initials_financing_p1",
            "initials",
            financing_page_1,
            312,
            1030,
        ...
        # Financing addendum page 2 buyer signature lines near bottom.
        # Adjusted after SignWell calibration QA: top buyer line is around y=825 in SignWell coordinates.
        add_field(
            "buyer1_signature_financing_addendum",
            "signature",
            financing_signature_page,
            130,
            820,
            recipient_id="1",
            width=145,
        ...
        add_field(
            "buyer1_date_financing_addendum",
            "date",
            financing_signature_page,
            286,
            820,
        ...
        if has_buyer2:
            add_field(
                "buyer2_signature_financing_addendum",
                "signature",
                financing_signature_page,
                130,
                878,
            ...
            add_field(
                "buyer2_date_financing_addendum",
                "date",
                financing_signature_page,
                286,
                878,
...
def create_signwell_signature_request(offer, pdf_bytes):
    ...
    payload = {
        ...
        "metadata": {
            ...
            "debug_payload": "bundle_v3_buyer_only_consolidated"
        }
    }
    ...
    try:
        ...
        if ok:
            return {
                "enabled": True,
                "ok": True,
                "mode": "bundle_v3_buyer_only_consolidated",
                "test_mode": SIGNWELL_TEST_MODE,
        ...
        return {
            "enabled": True,
            "ok": False,
            "mode": "bundle_v3_buyer_only_consolidated_failed",
            "test_mode": SIGNWELL_TEST_MODE,
            "field_count": len(fields[0]) if fields else 0,
            "signwell_error": data
        }
