...
def seller_temp_lease_pdf_path():
    """Locate the approved TREC 15-7 seller temporary lease source PDF."""
    return find_existing_pdf(
...
def build_pages_data(
    ...
    pages[3] = [
        ...
        (62,  133, ck(seller_disc == "exempt"), "check_small"),
...
def fill_and_merge(offer):
    ...
    if as_is in ["no", "repairs", "repair", "seller repairs", "sellerrepairs"] or str(s.get("repairsText") or "").strip():
        ...
        as_is = "yes"
    buyer_temp_requested = buyer_temp_lease_requested(s)
    seller_temp_requested = seller_temp_lease_requested(s)
    if buyer_temp_requested and seller_temp_requested:
        raise ValueError("Choose either the Buyer or Seller Temporary Residential Lease, not both.")
    if buyer_temp_requested or seller_temp_requested:
        possession = "lease"
    else:
        possession = "funding"
    ...
    if seller_temp_lease_attached:
        # TREC 15-7. In a buyer-side offer packet, Buyer is Landlord and Seller
        # is Tenant. Only Buyer/Landlord fields are sent through SignWell; the
        # Seller/Tenant execution fields intentionally remain for listing-side execution.
