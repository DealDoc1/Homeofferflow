...
def build_pages_data(
    ...
    pages[1] = [
        ...
        (101, 647, str(first_present(s.get("additionalEarnestDays"), s.get("additionalEarnestDeadlineDays"))) if has_positive_money(additional_earnest) else ""),
    ...
    pages[10] = [
        ...
        (50, 478, s.get("agentBrokerage", "") if has_buyer_agent else "", 7),
        (178, 449, s.get("agentBrokerLicense", "") if has_buyer_agent else "", 7),
        (150, 431, s.get("agentName", "") if has_buyer_agent else "", 7),
        (126, 413, s.get("teamName", "") if has_buyer_agent else "", 7),
        (150, 396, s.get("agentEmail", "") if has_buyer_agent else "", 7),
        (170, 378, s.get("agentPhone", "") if has_buyer_agent else "", 7),
        (420, 378, s.get("agentLicense", "") if has_buyer_agent else "", 7),
    ]
...
def fill_and_merge(offer):
    ...
    if lead_required and lead_path and os.path.exists(lead_path):
    ...
    if has_bkup and os.path.exists(BACKUP_PDF):
        ...
        bkup_pages = {
            0: [
                ...
                (365, 525, fmt_money(bkup_addl_earnest) if bkup_addl_earnest else ""),
                (72, 512, fmt_money(bkup_addl_option) if bkup_addl_option else ""),
                (270, 512, str(bkup_addl_days) if bkup_addl_days else ""),

                (215, 254, bkup_first_md),
                (345, 254, bkup_first_yy),

                (379, 176, bkup_term_md),
                (524, 176, bkup_term_yy),
            ],
            1: [
                (181, 745, addr_full, 8),
            ],
...
def build_signwell_fields(offer, pdf_bytes):
    ...
    lead_required = lead_required_from_offer(offer) and bool(lead_pdf_path())
    ...
    if non_realty_page:
        add_sig_date_pair("buyer1_non_realty_items_addendum", non_realty_page, 76, 709, 246, 709, "1")
        if has_buyer2:
            add_sig_date_pair("buyer2_non_realty_items_addendum", non_realty_page, 76, 791, 246, 791, "2")
    ...
    if hoa_page:
        add_sig_date_pair("buyer1_hoa_addendum", hoa_page, 112, 914, 266, 914, "1")
        if has_buyer2:
            add_sig_date_pair("buyer2_hoa_addendum", hoa_page, 112, 1007, 266, 1007, "2")

    # Sale of Other Property Addendum - buyer signatures only. No seller fields.
    if sale_page:
        add_sig_date_pair("buyer1_sale_other_property_addendum", sale_page, 112, 552, 266, 552, "1")
        if has_buyer2:
            add_sig_date_pair("buyer2_sale_other_property_addendum", sale_page, 112, 650, 266, 650, "2")
    ...
    if backup_signature_page:
        ...
        add_sig_date_pair("buyer1_backup_addendum", backup_signature_page, 112, 270, 266, 270, "1")
        if has_buyer2:
            add_sig_date_pair("buyer2_backup_addendum", backup_signature_page, 112, 402, 266, 402, "2")
