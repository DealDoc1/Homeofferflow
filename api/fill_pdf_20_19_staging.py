...
def build_pages_data(
    ...
    pages[1] = [
        ...
        # 17S: User review - 5A(1) days was a tad high; move down slightly.
        ...
        # 17S: user review - Seller title-policy X was too low; move up 10 points.
        (314, 350, ck(title_payer == "seller"), "check_small"),
        ...
        # 17S: user review - 6A(8)(ii) X was too high; move down.
        (75,  158, ck(title_amend in ["ii_buyer", "ii_seller"]), "check_small"),
        (435, 158, ck(title_amend == "ii_buyer"), "check_small"),
        # 17S: user review - 6A(8) Seller expense X was too high; move down.
        (499, 158, ck(title_amend == "ii_seller"), "check_small"),
    ]
    ...
    pages[4] = [
        ...
        # 17S: keep repair text as a readable one-line fit when possible, inside the repair blank.
        *wrapped_entries(89, 692, s.get("repairsText", "") if as_is == "repairs" else "", max_chars=125, line_gap=8, fs=7, max_lines=2),
    ...
    pages[10] = [
        ...
        # 17S: user review - move each buyer-broker contact item up 12 points.
        (66, 514, s.get("agentBrokerage", "") if has_buyer_agent else "", 8),
        (185, 474, s.get("agentBrokerLicense", "") if has_buyer_agent else "", 8),
        (154, 460, s.get("agentName", "") if has_buyer_agent else "", 8),
        (126, 445, s.get("teamName", "") if has_buyer_agent else "", 8),
        (154, 430, s.get("agentEmail", "") if has_buyer_agent else "", 8),
        (172, 415, s.get("agentPhone", "") if has_buyer_agent else "", 8),
        (435, 415, s.get("agentLicense", "") if has_buyer_agent else "", 8),
    ]
    ...
    if has_bkup and os.path.exists(BACKUP_PDF):
        ...
        bkup_pages = {
            0: [
                ...
                # 17S: Backup A(2) fields: lower money amounts into blanks and move days into the within-days blank.
                (322, 520, fmt_money(bkup_addl_earnest) if bkup_addl_earnest else ""),
                (485, 520, fmt_money(bkup_addl_option) if bkup_addl_option else ""),
                (326, 508, str(bkup_addl_days) if bkup_addl_days else ""),

                # 17S: Backup Paragraph G first-contract date cleanup.
                (255, 234, bkup_first_md),
                (350, 234, bkup_first_yy),
