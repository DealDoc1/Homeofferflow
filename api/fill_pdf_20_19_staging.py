...
def build_pages_data(
    ...
    pages[1] = [
        ...
        # 17T: professional polish - move 5A(1) additional earnest days up into the days blank.
        (101, 660, str(first_present(s.get("additionalEarnestDays"), s.get("additionalEarnestDeadlineDays"))) if has_positive_money(additional_earnest) else ""),
    ...
    pages[4] = [
        ...
        # 17T: repair text uses two readable lines inside the repair blank, above the instruction text.
        *wrapped_entries(89, 692, s.get("repairsText", "") if as_is == "repairs" else "", max_chars=78, line_gap=10, fs=8, max_lines=2),
    ...
    pages[10] = [
        ...
        # 17T: buyer broker block remapped to the actual Buyer-only row blanks.
        (95, 488, s.get("agentBrokerage", "") if has_buyer_agent else "", 9),
        (222, 448, s.get("agentBrokerLicense", "") if has_buyer_agent else "", 9),
        (172, 433, s.get("agentName", "") if has_buyer_agent else "", 9),
        (142, 418, s.get("teamName", "") if has_buyer_agent else "", 9),
        (172, 403, s.get("agentEmail", "") if has_buyer_agent else "", 9),
        (185, 388, s.get("agentPhone", "") if has_buyer_agent else "", 9),
        (436, 388, s.get("agentLicense", "") if has_buyer_agent else "", 9),
    ]
    ...
    if has_bkup and os.path.exists(BACKUP_PDF):
        ...
        bkup_pages = {
            0: [
                ...
                # 17T: Backup A(2) fields seated on the blanks.
                (322, 513, fmt_money(bkup_addl_earnest) if bkup_addl_earnest else ""),
                (485, 513, fmt_money(bkup_addl_option) if bkup_addl_option else ""),
                (360, 501, str(bkup_addl_days) if bkup_addl_days else ""),

                # 17T: Backup Paragraph G first-contract date placed in the dated blanks.
                (245, 244, bkup_first_md),
                (350, 244, bkup_first_yy),
