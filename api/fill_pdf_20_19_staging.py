...
def build_pages_data(
    ...
    pages[10] = [
        ...
        # 17Q: buyer-agent broker block remap - normal-size text on the actual buyer-agent rows.
        (105, 500, s.get("agentBrokerage", "") if has_buyer_agent else "", 8),
        (180, 463, s.get("agentBrokerLicense", "") if has_buyer_agent else "", 8),
        (150, 448, s.get("agentName", "") if has_buyer_agent else "", 8),
        (126, 433, s.get("teamName", "") if has_buyer_agent else "", 8),
        (150, 418, s.get("agentEmail", "") if has_buyer_agent else "", 8),
        (170, 403, s.get("agentPhone", "") if has_buyer_agent else "", 8),
        (420, 403, s.get("agentLicense", "") if has_buyer_agent else "", 8),
    ]
    ...
    if has_bkup and os.path.exists(BACKUP_PDF):
        ...
        bkup_pages = {
            0: [
                ...
                # 17Q: Backup A(2) field cleanup.
                (342, 522, fmt_money(bkup_addl_earnest) if bkup_addl_earnest else ""),
                (390, 509, fmt_money(bkup_addl_option) if bkup_addl_option else ""),
                (455, 509, str(bkup_addl_days) if bkup_addl_days else ""),

                # 17Q: Backup Paragraph G/H date cleanup.
                (255, 221, bkup_first_md),
                (498, 221, bkup_first_yy),
...
def build_signwell_fields(offer, pdf_bytes):
    ...
    if hoa_page:
        ...
        if has_buyer2:
            add_sig_date_pair("buyer2_hoa_addendum", hoa_page, 112, 924, 286, 924, "2")
    ...
    if sale_page:
        ...
        if has_buyer2:
            add_sig_date_pair("buyer2_sale_other_property_addendum", sale_page, 112, 805, 286, 805, "2")
