...
def build_pages_data(
    ...
    pages[4] = [
        ...
        *wrapped_entries(89, 687, s.get("repairsText", "") if as_is == "repairs" else "", max_chars=78, line_gap=10, fs=8, max_lines=2),
    ...
    pages[10] = [
        ...
        # 17U: full reset of Buyer-only broker block rows after visual audit.
        (95, 492, s.get("agentBrokerage", "") if has_buyer_agent else "", 8),
        (222, 462, s.get("agentBrokerLicense", "") if has_buyer_agent else "", 8),
        (172, 447, s.get("agentName", "") if has_buyer_agent else "", 8),
        (142, 432, s.get("teamName", "") if has_buyer_agent else "", 8),
        (172, 417, s.get("agentEmail", "") if has_buyer_agent else "", 8),
        (185, 402, s.get("agentPhone", "") if has_buyer_agent else "", 8),
        (436, 402, s.get("agentLicense", "") if has_buyer_agent else "", 8),
    ]
    ...
    if has_bkup and os.path.exists(BACKUP_PDF):
        ...
        bkup_pages = {
            0: [
                ...
                # 17U: Backup A(2) fields reset into their visible blanks.
                (337, 518, fmt_money(bkup_addl_earnest) if bkup_addl_earnest else ""),
                (492, 518, fmt_money(bkup_addl_option) if bkup_addl_option else ""),
                (250, 505, str(bkup_addl_days) if bkup_addl_days else ""),

                # 17U: user note - Paragraph G date was too high; move down and keep H unchanged.
                (245, 236, bkup_first_md),
                (350, 236, bkup_first_yy),
