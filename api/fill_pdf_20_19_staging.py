...
def build_pages_data(
    ...
    broker_fee_type = val_lower(first_present(s.get("brokerFeeType"), s.get("sellerBrokerCompType")))
    if broker_fee_type in ["dollar", "dollars", "amount", "$"]:
        broker_fee_type = "amount"
    elif broker_fee_type in ["percent", "percentage", "%"]:
        broker_fee_type = "percent"
    broker_fee_amount = first_present(
    ...
    pages[1] = [
        ...
        # 17R: User review - 5A(1) days was a tad high; move down slightly.
        (101, 654, str(first_present(s.get("additionalEarnestDays"), s.get("additionalEarnestDeadlineDays"))) if has_positive_money(additional_earnest) else ""),

        (86, 498, str(s.get("optionDays", "7"))),

        # 17R: seller-paid title policy X was too high in alternate-path test.
        (314, 340, ck(title_payer == "seller"), "check_small"),
    ...
    pages[4] = [
        ...
        # 17R: keep long repair text above the instruction line and out of Paragraph E.
        *wrapped_entries(89, 698, s.get("repairsText", "") if as_is == "repairs" else "", max_chars=92, line_gap=8, fs=6, max_lines=2),
    ...
    pages[10] = [
        ...
        # 17R: full buyer-agent broker block remap. Put values on the blanks, not over labels.
        (66, 502, s.get("agentBrokerage", "") if has_buyer_agent else "", 8),
        (185, 462, s.get("agentBrokerLicense", "") if has_buyer_agent else "", 8),
        (154, 448, s.get("agentName", "") if has_buyer_agent else "", 8),
        (126, 433, s.get("teamName", "") if has_buyer_agent else "", 8),
        (154, 418, s.get("agentEmail", "") if has_buyer_agent else "", 8),
        (172, 403, s.get("agentPhone", "") if has_buyer_agent else "", 8),
        (435, 403, s.get("agentLicense", "") if has_buyer_agent else "", 8),
    ]
...
def fill_and_merge(offer):
    ...
    if has_non_realty and os.path.exists(non_realty_pdf_path):
        ...
        non_realty_pages = {
            0: [
                (230, 652, addr_full, 8),
                # 17R: amount belongs in Paragraph A money blank after "$", not in the item-description area.
                (220, 620, fmt_money(first_present(s.get("nonRealtyAmount"), s.get("nonRealtyItemsAmount"), s.get("nonRealtyAdditionalSum"))) if first_present(s.get("nonRealtyAmount"), s.get("nonRealtyItemsAmount"), s.get("nonRealtyAdditionalSum")) else "", 8),
                # 17R: item text starts on the first item-description line.
                *wrapped_entries(62, 506, non_realty_description, max_chars=98, line_gap=11, fs=7, max_lines=12),
            ],
        }
    ...
    if has_bkup and os.path.exists(BACKUP_PDF):
        ...
        bkup_pages = {
            0: [
                ...
                # 17R: Backup A(2) field cleanup from rendered alternate-path review.
                (322, 534, fmt_money(bkup_addl_earnest) if bkup_addl_earnest else ""),
                (485, 531, fmt_money(bkup_addl_option) if bkup_addl_option else ""),
                (270, 510, str(bkup_addl_days) if bkup_addl_days else ""),

                # 17R: Backup Paragraph G date cleanup; year was too far right.
                (255, 230, bkup_first_md),
                (350, 230, bkup_first_yy),
...
def build_signwell_fields(offer, pdf_bytes):
    ...
    # 17R: Buyer 1 main signature was about 3 points too low.
    ...
    if non_realty_page:
        ...
        if has_buyer2:
            # 17R: Buyer 2 non-realty signature up 7 points.
    ...
    if hoa_page:
        ...
        if has_buyer2:
            # 17R: Buyer 2 HOA signature up 10 points.
    ...
    if sale_page:
        ...
        if has_buyer2:
            # 17R: Buyer 2 Sale of Other Property signature up 20 points.
