...
def find_existing_pdf(*names):
    """Find an addendum PDF in Vercel root or api directory using several likely names."""
    roots = [BASE_DIR, os.path.dirname(__file__)]
    for root in roots:
        for name in names:
            if not name:
                continue
    ...
    wanted = {str(n).lower().replace("_", "").replace("-", "").replace(" ", "") for n in names if n}
    for root in roots:
        try:
            for fname in os.listdir(root):
                key = fname.lower().replace("_", "").replace("-", "").replace(" ", "")
                if fname.lower().endswith(".pdf") and key in wanted:
...
def lead_required_from_offer(s):
    raw = val_lower(first_present(s.get("leadBuiltBefore1978"), s.get("leadBasedPaint"), s.get("leadRequired")))
    try:
        year_built = int(str(first_present(s.get("yearBuilt"), s.get("propertyYearBuilt"), "0")).strip() or "0")
    except Exception:
        year_built = 0
...
def lead_pdf_path():
    return find_existing_pdf(
        ...
        "lead_based_paint_56_0.pdf",
        "lead_based_paint.pdf",
        "lead_based_paint_addendum.pdf",
        "lead_paint_addendum.pdf",
        "seller_disclosure_lead_based_paint.pdf",
        "sellers_disclosure_lead_based_paint.pdf",
        "trec_56-0.pdf",
        "trec_56_0.pdf",
        "TREC_56-0.pdf",
        "TREC_56_0.pdf",
...
def normalize_appraisal(v):
    ...
    aliases = {
        ...
        "additionalrightto terminate": "additional",
...
def normalize_title_amendment(v):
    raw = str(v or "i").strip().lower().replace("_", "").replace("-", "").replace(" ", "")
    aliases = {
        "i": "i",
        "none": "i",
        "no": "i",
        ...
        "iiseller": "ii_seller",
        "amendbuyer": "ii_buyer",
        "amendseller": "ii_seller",
        "shortagesinarea buyer": "ii_buyer",
        "shortagesinareabuyer": "ii_buyer",
        "shortagesinarea seller": "ii_seller",
        "shortagesinareaseller": "ii_seller",
    }
    return aliases.get(raw, raw)
...
def build_pages_data(
    ...
    def normalize_comp_type(v):
        raw = val_lower(v)
        if raw in ["dollar", "dollars", "amount", "$"]:
            return "amount"
        if raw in ["percent", "percentage", "%"]:
            return "percent"
        return raw

    # 20-19 Paragraph 12B is two separate contribution paths.
    # 12B(1): Seller contributes toward compensation owed by Buyer to Buyer's broker.
    # 12B(2): Buyer contributes toward compensation owed by Seller to Seller's broker.
    # Do not treat a literal "0" as a real contribution.
    seller_broker_comp_type = normalize_comp_type(first_present(s.get("brokerFeeType"), s.get("sellerBrokerCompType")))
    seller_broker_comp_amount = first_present(
        s.get("brokerFeeAmount"),
        s.get("brokerCompAmount"),
        s.get("buyerBrokerFeeAmount"),
        s.get("sellerBrokerCompAmount"),
        s.get("sellerBrokerComp")
    )
    seller_broker_comp_percent = first_present(
        s.get("brokerFeePercent"),
        s.get("brokerCompPercent"),
        s.get("buyerBrokerFeePercent"),
        s.get("sellerBrokerCompPercent")
    )
    if not seller_broker_comp_type and (has_positive_money(seller_broker_comp_amount) or has_positive_money(seller_broker_comp_percent)):
        seller_broker_comp_type = "amount" if has_positive_money(seller_broker_comp_amount) else "percent"

    buyer_broker_comp_type = normalize_comp_type(s.get("buyerBrokerCompType"))
    buyer_broker_comp_amount = first_present(s.get("buyerBrokerCompAmount"), s.get("buyerToSellerBrokerCompAmount"))
    buyer_broker_comp_percent = first_present(s.get("buyerBrokerCompPercent"), s.get("buyerToSellerBrokerCompPercent"))
    if not buyer_broker_comp_type and (has_positive_money(buyer_broker_comp_amount) or has_positive_money(buyer_broker_comp_percent)):
        buyer_broker_comp_type = "amount" if has_positive_money(buyer_broker_comp_amount) else "percent"
    ...
    water_source = first_present(
        s.get("sellerWaterProvider"),
        s.get("waterSourceProvider"),
        s.get("waterProvider"),
        s.get("waterDisclosureProvider"),
        ""
    )
    ...
    pages[1] = [
        ...
        (342, 664, fmt_money(additional_earnest) if has_positive_money(additional_earnest) else ""),
        # 5A(1) additional earnest days belongs on the lower "within ___ days" blank.
        # 17U: user note - 5A(1) additional earnest days was too high; nudge down slightly.
        (101, 656, str(first_present(s.get("additionalEarnestDays"), s.get("additionalEarnestDeadlineDays"))) if has_positive_money(additional_earnest) else ""),
        ...
        (75,  162, ck(title_amend in ["ii_buyer", "ii_seller"]), "check_small"),
        (435, 162, ck(title_amend == "ii_buyer"), "check_small"),
        (499, 162, ck(title_amend == "ii_seller"), "check_small"),
    ]
    ...
    pages[4] = [
        ...
        # 17U: repair text stays readable and drops slightly inside the blank.
        *wrapped_entries(89, 694, s.get("repairsText", "") if as_is == "repairs" else "", max_chars=76, line_gap=9, fs=8, max_lines=2),
    ...
    seller_12b_active = has_buyer_agent and (
        (seller_broker_comp_type == "amount" and has_positive_money(seller_broker_comp_amount))
        or (seller_broker_comp_type == "percent" and has_positive_money(seller_broker_comp_percent))
    )
    buyer_12b_active = has_buyer_agent and (
        (buyer_broker_comp_type == "amount" and has_positive_money(buyer_broker_comp_amount))
        or (buyer_broker_comp_type == "percent" and has_positive_money(buyer_broker_comp_percent))
    )
    pages[6] = [
        (129, 751, addr_full),

        # 12B(1): Seller contribution toward Buyer broker compensation.
        (62, 646, ck(seller_12b_active), "check_small"),
        (286, 646, ck(seller_12b_active and seller_broker_comp_type == "amount"), "check_small"),
        (310, 644, fmt_money(seller_broker_comp_amount) if seller_12b_active and seller_broker_comp_type == "amount" else "", 8),
        (390, 646, ck(seller_12b_active and seller_broker_comp_type == "percent"), "check_small"),
        (420, 644, str(seller_broker_comp_percent) if seller_12b_active and seller_broker_comp_type == "percent" else "", 8),

        # 12B(2): Buyer contribution toward Seller broker compensation.
        (62, 630, ck(buyer_12b_active), "check_small"),
        (286, 630, ck(buyer_12b_active and buyer_broker_comp_type == "amount"), "check_small"),
        (310, 628, fmt_money(buyer_broker_comp_amount) if buyer_12b_active and buyer_broker_comp_type == "amount" else "", 8),
        (390, 630, ck(buyer_12b_active and buyer_broker_comp_type == "percent"), "check_small"),
        (420, 628, str(buyer_broker_comp_percent) if buyer_12b_active and buyer_broker_comp_type == "percent" else "", 8),
    ]
    ...
    pages[8] = [
        ...
        # Leases
        (62, 571, ck(possession == "lease" or truthy(s.get("buyerTemporaryLease"))), "check_small"),
    ...
    pages[10] = [
        ...
        # 17V: Buyer-only broker block reset. Use readable size and seat values on the actual blank lines.
        (93, 494, s.get("agentBrokerage", "") if has_buyer_agent else "", 9),
        (222, 464, s.get("agentBrokerLicense", "") if has_buyer_agent else "", 9),
        (172, 449, s.get("agentName", "") if has_buyer_agent else "", 9),
        (142, 434, s.get("teamName", "") if has_buyer_agent else "", 9),
        (172, 419, s.get("agentEmail", "") if has_buyer_agent else "", 9),
        (185, 404, s.get("agentPhone", "") if has_buyer_agent else "", 9),
        (436, 404, s.get("agentLicense", "") if has_buyer_agent else "", 9),
    ]
...
def fill_and_merge(offer):
    ...
    title_amend = normalize_title_amendment(first_present(s.get("titleAmendment"), s.get("titleAmendmentExpense"), s.get("titlePolicyAmendment"), "i"))
    survey      = s.get("survey", "sellerExisting")
    seller_disc_raw = val_lower(first_present(s.get("sellerDisclosure"), s.get("sellerDisclosureStatus"), "notReceived"))
    if seller_disc_raw in ["received", "yes", "true", "1"]:
        seller_disc = "received"
    elif seller_disc_raw in ["notrequired", "not_required", "exempt", "notapplicable", "na", "n/a"]:
        seller_disc = "exempt"
    else:
    ...
    if has_non_realty and os.path.exists(non_realty_pdf_path):
        ...
        non_realty_pages = {
            0: [
                ...
                (220, 617, fmt_money(first_present(s.get("nonRealtyAmount"), s.get("nonRealtyItemsAmount"), s.get("nonRealtyAdditionalSum"))) if first_present(s.get("nonRealtyAmount"), s.get("nonRealtyItemsAmount"), s.get("nonRealtyAdditionalSum")) else "", 8),
    ...
    if has_bkup and os.path.exists(BACKUP_PDF):
        ...
        bkup_pages = {
            0: [
                ...
                # 17V: Backup A(2) reset. First money blank is on line 1; option fee and days blanks are on line 2.
                (337, 514, fmt_money(bkup_addl_earnest) if bkup_addl_earnest else ""),
                (77, 502, fmt_money(bkup_addl_option) if bkup_addl_option else ""),
                (286, 502, str(bkup_addl_days) if bkup_addl_days else ""),

                # 17V: Paragraph G First Contract date seated in the dated ___, 20___ blanks.
                (114, 226, bkup_first_md),
                (334, 226, bkup_first_yy),
...
def build_signwell_fields(offer, pdf_bytes):
    ...
    if non_realty_page:
        add_sig_date_pair("buyer1_non_realty_items_addendum", non_realty_page, 76, 703, 246, 703, "1")
        if has_buyer2:
            # 17S: Buyer 2 non-realty signature up 7 points.
            add_sig_date_pair("buyer2_non_realty_items_addendum", non_realty_page, 76, 775, 246, 775, "2")
