...
def build_pages_data(
    ...
    pages[1] = [
        ...
        (101, 659, str(first_present(s.get("additionalEarnestDays"), s.get("additionalEarnestDeadlineDays"))) if has_positive_money(additional_earnest) else ""),
    ...
    pages[10] = [
        ...
        (105, 499, s.get("agentBrokerage", "") if has_buyer_agent else "", 7),
        (178, 464, s.get("agentBrokerLicense", "") if has_buyer_agent else "", 7),
        (150, 449, s.get("agentName", "") if has_buyer_agent else "", 7),
        (126, 434, s.get("teamName", "") if has_buyer_agent else "", 7),
        (150, 419, s.get("agentEmail", "") if has_buyer_agent else "", 7),
        (170, 404, s.get("agentPhone", "") if has_buyer_agent else "", 7),
        (420, 404, s.get("agentLicense", "") if has_buyer_agent else "", 7),
    ]
    ...
    if has_non_realty and os.path.exists(non_realty_pdf_path):
        ...
        non_realty_pages = {
            0: [
                ...
                *wrapped_entries(62, 501, non_realty_description, max_chars=88, line_gap=11, fs=7, max_lines=12),
            ],
        }
    ...
    if has_bkup and os.path.exists(BACKUP_PDF):
        ...
        bkup_pages = {
            0: [
                ...
                (82, 512, fmt_money(bkup_addl_option) if bkup_addl_option else ""),
                (274, 512, str(bkup_addl_days) if bkup_addl_days else ""),

                (215, 236, bkup_first_md),
                (345, 236, bkup_first_yy),
...
def build_signwell_fields(offer, pdf_bytes):
    ...
    if non_realty_page:
        ...
        if has_buyer2:
            add_sig_date_pair("buyer2_non_realty_items_addendum", non_realty_page, 76, 782, 246, 782, "2")
    ...
    if hoa_page:
        add_sig_date_pair("buyer1_hoa_addendum", hoa_page, 112, 870, 266, 870, "1")
        if has_buyer2:
            add_sig_date_pair("buyer2_hoa_addendum", hoa_page, 112, 958, 266, 958, "2")

    # Sale of Other Property Addendum - buyer signatures only. No seller fields.
    if sale_page:
        add_sig_date_pair("buyer1_sale_other_property_addendum", sale_page, 112, 620, 266, 620, "1")
        if has_buyer2:
            add_sig_date_pair("buyer2_sale_other_property_addendum", sale_page, 112, 718, 266, 718, "2")
    ...
    if backup_signature_page:
        ...
        add_sig_date_pair("buyer1_backup_addendum", backup_signature_page, 112, 230, 266, 230, "1")
        if has_buyer2:
            add_sig_date_pair("buyer2_backup_addendum", backup_signature_page, 112, 362, 266, 362, "2")
