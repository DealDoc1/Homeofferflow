...
def build_signwell_fields_txr1508(data, *, client_count=1):
    ...
    fields = [
        # The agent acknowledgement has its own initials rule on the left of
        # the row.  Customer initials use the right-side rule below; sharing
        # their x-coordinate places the agent widget in the gap before Date.
        {"api_id": "txr1508_agent_initials_p1", "type": "initials", "page": 1, "x": 370, "y": 672, "recipient_id": "associate" if signer_plan == "associate_and_clients" else "broker", "required": True, "width": 72, "height": 18},
